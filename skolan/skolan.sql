...
/*!50001 CREATE VIEW `v_planering` AS SELECT
 1 AS `akronym`,
  1 AS `Namn`,
  1 AS `antal_kurstillfallen` */;
SET character_set_client = @saved_cs_client;

--
-- Dumping routines for database 'skolan'
--
...
/*!50001 VIEW `v_larare` AS select `larare_pre`.`akronym` AS `akronym`,`larare_pre`.`avdelning` AS `avdelning`,`larare_pre`.`fornamn` AS `fornamn`,`larare_pre`.`efternamn` AS `efternamn`,`larare_pre`.`kon` AS `kon`,`larare_pre`.`lon` AS `lon`,`larare_pre`.`fodd` AS `fodd`,`larare_pre`.`kompetens` AS `kompetens`,timestampdiff(YEAR,`larare_pre`.`fodd`,curdate()) AS `alder` from `larare_pre` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`dbadm`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_lonerevision` AS select `l`.`akronym` AS `akronym`,`l`.`fornamn` AS `fornamn`,`l`.`efternamn` AS `efternamn`,`p`.`lon` AS `pre`,`l`.`lon` AS `nu`,`l`.`lon` - `p`.`lon` AS `diff`,`p`.`kompetens` AS `prekomp`,`l`.`kompetens` AS `nukomp`,`l`.`kompetens` - `p`.`kompetens` AS `diffkomp`,round((`l`.`lon` - `p`.`lon`) / `p`.`lon` * 100,2) AS `proc`,case when (`l`.`lon` - `p`.`lon`) / `p`.`lon` * 100 >= 3 then 'ok' else 'nok' end AS `mini` from (`larare` `l` join `larare_pre` `p` on(`l`.`akronym` = `p`.`akronym`)) order by round((`l`.`lon` - `p`.`lon`) / `p`.`lon` * 100,2) desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`dbadm`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_namn_alder` AS select concat(`larare_pre`.`fornamn`,' ',`larare_pre`.`efternamn`,' (',lcase(`larare_pre`.`avdelning`),')') AS `namn`,timestampdiff(YEAR,`larare_pre`.`fodd`,curdate()) AS `alder` from `larare_pre` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`dbadm`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_planering` AS select `l`.`akronym` AS `akronym`,concat(`l`.`fornamn`,' ',`l`.`efternamn`) AS `Namn`,count(`k`.`kurskod`) AS `antal_kurstillfallen` from (`larare` `l` join `kurstillfalle` `k` on(`l`.`akronym` = `k`.`kursansvarig`)) group by `l`.`akronym`,concat(`l`.`fornamn`,' ',`l`.`efternamn`) order by count(`k`.`kurskod`) desc,`l`.`akronym` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-11-06  3:42:45
