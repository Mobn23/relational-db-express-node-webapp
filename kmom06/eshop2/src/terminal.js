...
async function showMenu() {
    console.table(`
        ...
        order          Display orders
        order <order or costumer id>    Displays a specific order
        picklist <orderid>  genererar en plocklista för vald order och dess orderrader
...
async function showOrders() {
    const db = await mysql.createConnection(config);
    const sql = `CALL show_orders_cli();`;
    const res = await db.query(sql);

    console.table(res[0]);
}

async function showSpecificOrder(orderIdOrCustomerId) {
    const db = await mysql.createConnection(config);
    const sql = `CALL show_orders_cli_by_arg(?);`;
    const res = await db.query(sql, [ orderIdOrCustomerId ]);

    console.table(res[0]);
}

async function generatePlockList(orderId) {
    const db = await mysql.createConnection(config);
    const sql = `CALL generate_picklist(?);`;
    const res = await db.query(sql, [ orderId ]);

    console.table(res[0]);
}

async function changeStatusToShiped(orderId) {
    const db = await mysql.createConnection(config);
    const sql = `CALL change_status_to_shiped(?);`;
